...
**本番稼働実績（2026-07-15〜07-24・§8再校正の入力データ）**：
- 平日8営業日の scheduled run **すべて成功**（07-15/16/17/20/21/22/23/24）。誤 fail ゼロ。
- **2026-07-20（海の日・東証休場）の run で `stale_price` warn が本番発火**：全299銘柄が
  前営業日と同値 → warn集約 Issue #2 に自動追記。「祝日は正常に一致する→warn止まりにする
  （配信は継続）」という設計判断が実際の祝日で検証された。誤検知ではなく事実の検知。
- ci-verify ブランチでのフル実行検証（07-15 07:30 JST・東証開場前）でも同様に
  stale_price warn＋warn-notify 経路の実動作を確認済み（ブランチは検証後に削除）。
- ドリル Issue #1 は通知到達をユーザー本人が確認しクローズ済み。**warn集約 Issue #2 は
  意図的にオープンのまま維持**（オープン中の data-warn Issue に追記する集約設計のため、
  これが「warnの台帳」になる。クローズすると次の祝日warnが新規Issueとして乱立する）。
- cron遅延の実測が2点追加：07-20 run は予定16:30 JSTに対し開始 00:22 JST（+7.8h）。
  鮮度バナー96hの再校正材料として記録。
